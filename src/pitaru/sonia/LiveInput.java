...
public class LiveInput {
	...
	public static void start(int rate, int nSamples) {
		...
		if (micFlag) {
			if (Synth.openCount == 1) {
				...
				Synth.start(0, rate, Sonia.inDevID, Sonia.inDevChNum, Sonia.outDevID,
						Sonia.outDevChNum);
				BJSyn.startEngine();
	...
	public static float getLevel() {
		...
		if (followerL != null && followerL.output != null) {
			level += followerL.output.get();
			count++;
		}
		if (followerR != null && followerR.output != null) {
			level += followerR.output.get();
			count++;
		}
		
		return count > 0 ? (level / (float)count) : 0f;
	}
