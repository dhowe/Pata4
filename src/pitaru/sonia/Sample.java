...
public class Sample {
		...
		public Sample(String fileName){
			...
			//if (Synth.openCount == 0) Sonia.initJsyn(44100,0);
			...
			if(BJSyn.count == BJSyn.maxSamples) BJSyn.count = 0;
		...
		public void initEmptySample(int frameNum, int _rate, int _channels){
			...
			id = BJSyn.count;
			channels = Math.max(Math.min(_channels,2),1); //channels = BJSyn.channelNum;
			...
			for(int i=0; i<channels; i++){
				...
				setRate(rate,i);
				if(channels > 1){
					if(i == 0) setPan(-1 ,i); else setPan(1 ,i);
				}
			}
		...
		public void setVolume(float v){
			...
			if(channels == 2) BJSyn.setVolume(id+1, v);
		}
		...
		public void setRate(float v){
			BJSyn.setRate(id, v);
			if(channels == 2) BJSyn.setRate(id+1,v);
		}
		...
		public void setSpeed(float v){
			...
			if(channels == 2) BJSyn.setRate(id+1,(float)(v*rate));
		}
		...
		public void repeat(int start, int end){
				...
				if(channels == 2)	BJSyn.loopSample(id+1, start, end);
		...
		public void repeatNum(int num, int start, int end){
				...
				if(channels == 2)	BJSyn.loopSampleNum(num, id+1, start, end);
		}
		...
		public void play(int start, int end){
				...
				if(channels == 2) BJSyn.playSample(id+1, start, end);
		...
		public void stop(boolean stopFlag, int stopOffset){
			...
			if(channels == 2) BJSyn.stopSample(id+1,stopFlag, stopOffset );
		}
		...
		public void stop(){
			stop(true,0); // used to be false - changed because smaple doesn't stop on xp
		}

		public void delete(){
			stop(0);
			BJSyn.deleteCircuit(id);
			if(channels == 2) BJSyn.deleteCircuit(id+1);
		}
...
// READ
	// FLOATS
		...
		public void readChannel(int part, float[] data, int firstDataFrame, int firstSampleFrame, int numFrames){
		...
		public void read(float[] data, int firstDataFrame, int firstSampleFrame, int numFrames){
	...
	// SHORT
		...
		public void readChannel(int part, short[] data_s, int firstDataFrame, int firstSampleFrame, int numFrames){
					BJSyn.mySamp[id+part].read(firstSampleFrame, data_s, 0, numFrames);
		}
		...
		public void read(short[] data_s, int firstDataFrame, int firstSampleFrame, int numFrames){
...
// WROTE
	//FLOAT
		...
		public void writeChannel(int part, float[] data, int firstDataFrame, int firstSampleFrame, int numFrames){
					BJSyn.mySamp[id+part].write(firstSampleFrame, convertData(data), 0, numFrames);
		}
		...
		public void write(float[] data, int firstDataFrame, int firstSampleFrame, int numFrames){
	...
	//SHORT
		...
		public void writeChannel(int part, short[] data_s, int firstDataFrame, int firstSampleFrame, int numFrames){
		...
		public void write(short[] data_s, int firstDataFrame, int firstSampleFrame, int numFrames){
		...
		static public void convertData(short[] data_s, float[] data, int numFrames, int firstFrame){
					for ( int i = 0; i < numFrames; i++){
...
int lastFrame = 0;
		...
		public boolean isPlaying(){
			switch(state){
				...
				return false;
				case 1:
					int test = (startF + getRangeFrames() - getCurrentFrame() );
