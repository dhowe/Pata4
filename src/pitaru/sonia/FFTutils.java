...
class FFTutils {
  ...
  float[] computeFFT(float[] waveInData) {
    ...
    if (_isEqualized) {
      for(i=0; i<WS2; i++) _fft_result[i]=_equalize[i]*(float)Math.sqrt(_fftBuffer[i][0]*_fftBuffer[i][0]+_fftBuffer[i][1]*_fftBuffer[i][1]);
    } else {
      for(i=0; i<WS2; i++) _fft_result[i]=_normF*(float)Math.sqrt(_fftBuffer[i][0]*_fftBuffer[i][0]+_fftBuffer[i][1]*_fftBuffer[i][1]);
    }
    return _fft_result;
  }
